...
# About the weights
# 在cor函数中，weights.x和weights.y是两个用来指定变量x和变量y中每个观测值的权重的参数。在计算相关系数时，这些权重可以影响每个观测值的重要性，因此可以用来调整不同观测值之间的相对影响，从而得到更准确的相关系数。
# 如果不提供权重参数，则默认所有的权重都是1，即所有的观测值都被视为具有相等的权重。如果提供了权重参数，则权重参数必须是一个和x或y具有相同行数和列数的矩阵，其中每个元素表示相应观测值的权重值。如果提供的权重值不是非负数，则会产生错误。如果有非有限权重值，则这些值将被视为缺失值，并被设为NA。
# 所以这个相关性的做法中是对每个样本都有相应的权重，才做
