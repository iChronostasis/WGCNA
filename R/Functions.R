...
# 这是一个名为 scaleFreeFitIndex 的函数，用于计算一个网络的“拟合指数”。该函数有三个输入参数：节点的度数向量 k、将度数分段的数量 nBreaks（默认为10），以及一个逻辑值 removeFirst，指示是否要移除第一个度数分段（默认为 FALSE）。

# 该函数首先将度数向量 k 分成 nBreaks 个分段，并计算每个分段中度数的平均值和占总度数的比例。然后，它将 k 绘制成直方图，并从直方图的中点获取度数的值，以便计算一个截断指数。如果存在缺失值，函数将使用直方图的中点来代替相应的度数平均值。

# 在计算截断指数之前，函数还可以选择删除第一个度数分段。最后，函数将使用线性回归来拟合度数分布的幂律，并计算 $R^2$ 值、斜率和调整后的 $R^2$ 值，并将它们存储在一个数据框中，然后返回该数据框作为函数的输出。

# 函数首先将k向量离散化为nBreaks个级别，并计算每个级别的平均度数dk和该级别占所有节点数的比例p.dk。然后，将dk和p.dk的值取log10，从而将线性拟合转换为线性回归问题。
#========================================================================================================
